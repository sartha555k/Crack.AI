  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 max-w-md w-full overflow-hidden transform animate-in zoom-in slide-in-from-bottom-8 duration-500 ease-out">
        <div
          className={`h-2 w-full ${type === "danger" ? "bg-rose-500" : "bg-teal-500"}`}
        ></div>

        <div className="p-8 sm:p-10 text-center">
          <div
            className={`w-16 h-16 mx-auto mb-6 rounded-2xl flex items-center justify-center text-2xl ${type === "danger" ? "bg-rose-50/80 text-rose-500" : "bg-teal-50/80 text-teal-500"}`}
          >
            {type === "danger" ? "🗑️" : "⚠️"}
          </div>

          <h3 className="text-2xl font-black text-slate-900 mb-2 tracking-tight uppercase px-2">
            {title}
          </h3>
