...
function InterviewRunner() {
  ...
  const isWaitingForAI = useMemo(() => {
    ...
    if (activeSession?.questions?.length <= currentQuestionIndex + 1)
      return true;
    return false;
  }, [
    isQuestionLocked,
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === "hidden" && hasJoined && !isTerminated) {
        ...
        toast.error("Integrity Warning: Focus Lost!", {
          position: "top-center",
        });
      }
    };
    ...
    const poll = setInterval(async () => {
      if (window.tf && window.cocoSsd) {
        ...
        try {
          ...
          setProctorStatus("AI PROCTOR: ERR");
        }
      }
    }, 1000);
  ...
  useEffect(() => {
    if (sessionId) dispatch(getSessionById(sessionId));
  }, [dispatch, sessionId]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (hasJoined && activeSession?.startTime) {
      ...
      sessionTimerRef.current = setInterval(() => {
        ...
        const effectiveNow =
          activeSession.isPaused && activeSession.lastPauseStart
            ? new Date(activeSession.lastPauseStart).getTime()
            : Date.now();

        const totalElapsedSec = Math.max(
          0,
          (effectiveNow - startTimeMs - pauseOffsetMS) / 1000,
        );
    ...
    activeSession?.pauseTimeMS,
    activeSession?.isPaused,
    activeSession?.lastPauseStart,
    isTerminated,
  ]);
  ...
  const handleSubmit = async () => {
    ...
    if (!draft?.code && !draft?.audioBlob) {
      toast.warning("Submit answer.");
      return;
    }
    ...
    if (draft.audioBlob)
      formData.append("audioFile", draft.audioBlob, "answer.webm");
  ...
  if (!activeSession)
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900 text-teal-400 font-bold">
        RECONSTRUCTING DATA...
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center animate-in fade-in zoom-in duration-700">
        <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border border-slate-100 max-w-2xl w-full">
          <div className="w-20 h-20 bg-teal-500/10 text-teal-600 rounded-3xl flex items-center justify-center text-4xl mb-8 mx-auto">
            🎉
          </div>
          ...
          <p className="text-slate-500 mb-10 font-medium leading-relaxed">
            Thank you for your time and effort. We have logged your performance,
            integrity metrics, and AI technical evaluations. A comprehensive
            report is now ready for your review.
          </p>
          <button
            onClick={() => navigate(`/review/${sessionId}`)}
            className="w-full py-5 bg-teal-600 text-white font-black text-xl rounded-3xl hover:bg-teal-500 transition-all shadow-xl shadow-teal-500/20 active:scale-95 uppercase tracking-widest"
          >
            View Global Analysis
          </button>
        </div>
        <p className="mt-10 text-[10px] font-black text-slate-300 uppercase tracking-widest">
          Assessment System v2.0 • Secured by AI Proctor
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white p-6">
        ...
        <p className="text-slate-400 mb-10 max-w-sm text-center">
          The timer will only start ticking once you join. All AI processing
          time is excluded.
        </p>
        {!(activeSession?.questions?.length > 0) ? (
          ...
          <button
            onClick={handleJoin}
            className="px-16 py-5 bg-teal-600 text-white font-black text-xl rounded-2xl hover:bg-teal-500 transition-all shadow-xl shadow-teal-500/20 active:scale-95 uppercase tracking-widest"
          >
            ENTER INTERVIEW
          </button>
        )}
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      <div className="bg-slate-900 text-white px-6 py-4 flex justify-between items-center z-20 sticky top-0 shadow-xl">
        <div className="flex items-center gap-4">
          <div
            className={`w-12 h-12 bg-white/5 flex items-center justify-center rounded-xl border border-white/10 ${isSpeaking ? "ring-2 ring-teal-500" : ""}`}
          >
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${proctorStatus.includes("ACTIVE") ? "bg-emerald-500 text-slate-900" : "bg-slate-700 text-slate-300"}`}
              >
                {proctorStatus}
              </span>
              {isWaitingForAI && (
                <span className="text-[9px] font-black uppercase bg-amber-500 text-slate-900 px-2 py-0.5 rounded animate-pulse">
                  CLOCK PAUSED
                </span>
              )}
              {violationCount > 0 && (
                <span className="text-[9px] font-black uppercase bg-rose-600 text-white px-2 py-0.5 rounded">
                  Security Logged
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-6">
          <div
            className={`flex flex-col items-end pr-4 border-r border-white/10`}
          >
            <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
              Time Remaining
            </span>
            <span
              className={`font-mono text-2xl font-black tabular-nums transition-colors ${timeLeft < 120 ? "text-rose-500" : "text-white"}`}
            >
              {formatTime(timeLeft)}
            </span>
          </div>
          <button
            ...
            className="text-[10px] font-black bg-rose-600/20 text-rose-500 border border-rose-600/50 hover:bg-rose-600 hover:text-white px-3 py-2 rounded-lg tracking-widest"
          >
            QUICK EXIT
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="bg-black rounded-[2.5rem] overflow-hidden shadow-2xl relative border-4 border-white aspect-video bg-slate-950">
              <video
                ref={(v) => {
                  if (v) {
                    videoRef.current = v;
                    if (videoStreamRef.current && !v.srcObject)
                      v.srcObject = videoStreamRef.current;
                  }
                }}
                autoPlay
                muted
              ...
              />
              {proctorWarning && (
                <div className="absolute inset-x-0 bottom-0 bg-rose-600 font-bold p-3 text-center text-white text-[11px] uppercase z-10 animate-pulse">
                  {proctorWarning}
                </div>
              )}
            </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col min-h-[250px] relative overflow-hidden">
              ...
              <span className="text-[10px] font-black text-teal-600 uppercase tracking-[0.2em] mb-4 border-l-2 border-teal-500 pl-3">
                Live Question
              </span>
              <h2 className="text-xl font-bold text-slate-800 leading-relaxed whitespace-pre-wrap flex-1 z-10">
                {currentQuestion?.questionText}
              </h2>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            <div className="bg-white p-6 rounded-[2rem] border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
              <div className="flex items-center gap-4 font-black text-slate-400 uppercase tracking-widest text-[10px]">
                🎙️ Verbal Response
              </div>
              <div className="flex items-center gap-6">
                {!isRecording && !currentDraft.audioBlob ? (
                  <button
                    ...
                    className="px-8 py-3 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95 disabled:opacity-30"
                  >
                    Record Answer
                  </button>
                ) : isRecording ? (
                  <div className="flex items-center gap-4">
                    ...
                    <button
                      ...
                      className="px-8 py-3 bg-rose-600 text-white rounded-full text-[10px] font-black uppercase animate-pulse"
                    >
                      Stop Recording
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-3 px-6 py-3 bg-emerald-50 rounded-full border border-emerald-100">
                    ...
                    {!isQuestionLocked && (
                      <button
                        ...
                        className="text-rose-500 text-[9px] font-black uppercase hover:underline ml-2"
                      >
                        Delete
                      </button>
                    )}
            ...
            <div
              className="bg-[#1e1e1e] rounded-[2.5rem] shadow-2xl flex flex-col border border-black overflow-hidden"
              style={{ height: "380px" }}
            >
              <div className="flex justify-between items-center px-8 py-3 bg-[#181818] border-b border-black">
                ...
                <select
                  ...
                  disabled={isQuestionLocked}
                  className="text-[10px] bg-[#2d2d2d] text-slate-300 border-none rounded-lg px-4 py-1.5 font-bold outline-none cursor-pointer"
                >
